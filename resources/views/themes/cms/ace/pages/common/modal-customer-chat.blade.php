                    ...
                    <div class="card m-b-0" style="background: transparent;">
                        ...
                        <div class="chat-main-box">
                            ...
                            <div class="chat-left-aside" style="width: calc(100% - 1px);">
                                ...
                                <div class="card-body border-top chat-control-abs chat-order box-submit">
                                    <div class="row">
...
<script>
    function showChat(order_id, name_service){
            $('#data-loading').show();
            $('#ajax_show_chat').html('');
            ...
            $.ajax({
                url: '{{ route('customer_order_chat_ajax') }}',
                type: 'POST',
                dataType: 'HTML',
                data: {
                    _token: '{{ csrf_token() }}',
                ...
                success: function(result) {
                    $('#data-loading').hide();
                    //
                    let total_no_seen_sale = document.getElementById('total_no_seen_sale_'+ order_id);
                    total_no_seen_sale.innerHTML = '';
                    $('#ajax_show_chat').html(result);
                    $('#chat_modal').modal('show');
                    setTimeout(function() {
                    let height = document.getElementsByClassName("chat-main-box")[0].offsetHeight - document.getElementsByClassName("box-submit")[0].offsetHeight - 5 - 178;
