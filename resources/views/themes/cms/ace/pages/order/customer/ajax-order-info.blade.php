    ...
    <div class="table-responsive">
        <table class="table table-custome table-bordered table-small mb-0">
            <tbody>
                ...
                <tr>
                    {{-- Send Request --}}
                    <td class="name font-weight-500">{{ trans('fotober.order.order_submited') }}</td>
...
<div class="modal-footer">
    ...
    <a href="{{ route('customer_order_edit', ['id' => $order->id]) }}" class="btn btn-info waves-effect btn-w125" onclick="return confirm('{{ trans('fotober.common.confirm_perform') }}');" title="Click to Edit Order">
        {{ trans('fotober.common.btn_edit') }}
        <i class="ti-angle-right" style="font-size: 12px;"></i>
    </a>&nbsp;
    <a href="{{ route('customer_order_send_request', ['id' => $order->id]) }}" class="btn btn-info waves-effect btn-w125" onclick="return confirm('{{ trans('fotober.common.confirm_perform') }}');" title="Click to Place Order">
        {{ trans('fotober.order.btn_place_order') }}
        <i class="ti-angle-right" style="font-size: 12px;"></i>
    </a>
    @else
    <!-- <a href="javascript:void(0)" onclick="showChat({{$order->id}},'{{ $order->service->name }}')" class="btn btn-info waves-effect">
