<div class="table-responsive">
    <table id="simple-table" class="table table-hover table-centered table-order table-head-bg">
        ...
        <tr>
            ...
            <th class="name">{{ trans('fotober.payment.name') }}</th>
            <th class="name">{{ trans('fotober.payment.delivery') }}</th>
        ...
        <tr>
            ...
            <td>
                @if(\Illuminate\Support\Facades\Auth::user()->account_type == \App\Helpers\Constants::ACCOUNT_TYPE_CUSTOMER)
                    @if (in_array($item->status, [\App\Helpers\Constants::PAYMENT_STATUS_NEW, \App\Helpers\Constants::PAYMENT_STATUS_FALIED]))
                        <a href="https://www.paypal.com/vn/signin" target="_blank">
